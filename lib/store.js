let store = (function(initialData = {}) {
  ...
  let history = (function() {
    let send = function (data) {
      proxy['__HISTORY__'] = { source: this, data }
    };
  ...
  let proxy = new Proxy(data, {
    set: function (target, propertyKey, newVal)  {
      if(propertyKey === '__INIT__') {
        // set value in history
        Object.keys(newVal.value).map(key => {
          data[key] = { source: '__INIT__', value: newVal.value[key] }
        });
        history.receive({ data: { ...data }, changed: { source: '__INIT__' }});
        // set value to subscribers
        subscribers.forEach(o => o.receive({ data: { ...data }, changed: { source: '__INIT__' }} ))
        return;
      }
      else if(propertyKey === '__HISTORY__') {
  ...
  window.setTimeout(() => {
    store.dispatch(() => {
      return { key: '__INIT__', value: initialData }
    });
  }, 0)

  return Object.freeze(store);
})({ text1: 'initial-text-1', text2: 'initial-text-2' });
